...
class MplCanvas(FigureCanvas):
    ...
    def dibujar_grafo(self, grafo, ruta_alternativa=None):
        ...
        for nodo, data in grafo.grafo.nodes(data=True):
            if data["tipo"] == "tanque":
                if data["nivel"] > data["capacidad"]:
                    colores.append("red")  # Tanque desbordado
                elif data["nivel"] < data["capacidad"] * 0.1:
                    colores.append("purple")  # Tanque casi vacío
                else:
                    colores.append("blue")
                tamanos.append(600)
            else:  # Si es una casa
        ...
        # Configuración de colores de las aristas
        colores_aristas = {
            (u, v): (
        ...
        # Dibujar el grafo
        nx.draw(
            grafo.grafo, pos, with_labels=True, node_color=colores,
            node_size=tamanos, font_weight="bold", ax=self.axes,
        ...
        etiquetas_nodos = {
            nodo: (
                f"\n\n\nNivel: {data['nivel']}/{data['capacidad']}L" if data["tipo"] == "tanque" else
                f"\n\n\nDemanda: {data['demanda']}L/s"
            )
            for nodo, data in grafo.grafo.nodes(data=True)
        }

        nx.draw_networkx_labels(grafo.grafo, pos, labels=etiquetas_nodos, font_size=10, ax=self.axes)
...
class Interfaz(QMainWindow):
    def __init__(self):
        ...
        layout.addWidget(agregar_casa_btn)

        # Botón para agregar tubería
        agregar_tuberia_btn = QPushButton("Agregar Tubería")
        agregar_tuberia_btn.clicked.connect(self.agregar_tuberia)
        layout.addWidget(agregar_tuberia_btn)

        # Botón para agregar tanque con capacidad
        ...
        layout.addWidget(cambiar_sentido_btn)
        
        # Botón para verificar suministro
        ...
        layout.addWidget(verificar_suministro_btn)
    ...
    def verificar_suministro(self):
        ...
        if not resultados:
            ...
            QMessageBox.warning(
                self, "Problemas de Suministro", "Algunas casas no tienen suficiente agua. Revisa las sugerencias para más detalles."
            )
